...
class GameRulezInPython:
	class State:
		pass

	def __init__(self):
		self.ActionMap = {'noop' : GameRulezInPython.actionNoop, 'play' : GameRulezInPython.actionPlay, 'take' : GameRulezInPython.actionTake }
	
	def start(self, numPlayers,allcards=None):
		#fluents : stack : list, hand : list, known : list
		state = GameRulezInPython.State()
		state.stack=[]
		state.hand=[]
		state.known=[]
	...
	def getLegalMoves(state, playerIdx):
		...
		if ss == 0:
			...
			if imin < len(hand):
				...
				quads = GameRulezInPython.findColorQuadsToPlay(hand, tos)
	...
	def actionTake(state, pIdx, cnt):
		nextState = GameRulezInPython.State()
		...
		for pi in range(0,len(state.hand)):
			if pi != pIdx:
				...
				nextState.known[pi] = sorted(GameRulezInPython.replaceZeros(state.known[pi],cards))
		return nextState
...
def test():
	gr = GameRulezInPython()
	assert(  GameRulezInPython.replaceZeros([0,0,0,0,0,91,92,93],[101,102]) == [0,0,0,91,92,93,101,102] )
	assert(  GameRulezInPython.replaceZeros([0,91,92,93],[101,102]) == [91,92,93,101,102])
	assert(  GameRulezInPython.replaceZeros(  [91,92,93],[101,102]) == [91,92,93,101,102])
